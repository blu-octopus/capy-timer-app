...
selection, phase, success, denied, unlock), so call sites never coordinate two systems.
`components/ui/Button` and `IconButton` intercept `onPress`, which covers nearly every control;
both take a `feedback` prop to change the kind or opt out. Phase boundaries and completion go
through `hooks/useRunFeedback.ts`, which subscribes to `[status, phase, loopIndex]` for the same
reason `useSessionNotifications` does. `denied`/`unlock` are the one pair wired ad hoc, at the
two outcomes of `app/index.tsx`'s companion-purchase flow (insufficient coins vs. a completed
unlock) rather than through a shared hook, since there's no store-state transition to subscribe
to the way phase/status changes give the other kinds one. Sounds are generated by `npm run sfx`
(`scripts/generate-sfx.mjs`). Nothing in this path may throw or block: haptics are skipped on
web, and audio goes permanently quiet after a construction failure rather than retrying on
every tap.

`generate-sfx.mjs` has two synthesis engines: the original sine-partial one (`tap`/`tick`/
`phase`/`complete`) and a layered one added afterward (`denied`/`unlock`) supporting filtered
noise, pitch glide, and a shimmer/echo tail — written after reading
[cuelume](https://github.com/Danilaa1/cuelume), a web-only Web Audio library (`window.AudioContext`,
DOM `data-*` attributes) that can't run on Hermes/React Native at all and was evaluated and
rejected as a dependency for exactly that reason. What carried over was its synthesis
vocabulary, reimplemented as offline sample-buffer math since there's no live audio graph to
render through here, only a WAV file to write once. See the file's own top comment for the
full reasoning.
